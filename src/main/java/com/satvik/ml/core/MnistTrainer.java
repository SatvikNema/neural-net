...
public class MnistTrainer implements NeuralNetworkTrainer {
    ...
    for (int t = 0; t < iterations; t++) {
      for (Pair<Matrix, Matrix> trainingDatum : trainingData) {
        ...
        double errorAdditionTerm =
            neuralNetwork.getOutputErrorDiff().apply(x -> x * x).sum() / trainingData.size();
  ...
  private NeuralNetwork trainForOneInput(
      Pair<Matrix, Matrix> trainingData, NeuralNetwork nnConfig) {
    ...
    Matrix outputLayerErrorTerm =
        nnConfig.getLayerOutputs().get(layerInProcessing).subtract(trainingData.getB());
    Matrix deltaWeightLast =
        outputLayerErrorTerm.multiply(
            nnConfig.getLayerOutputs().get(layerInProcessing - 1).apply(Functions::sigmoid));

    Matrix newWeights =
        nnConfig
            .getWeights()
            .get(layerInProcessing)
            .subtract(deltaWeightLast.apply(x -> learningRate * x));
    ...
    Matrix newBiases =
        nnConfig
            .getBiases()
            .get(layerInProcessing)
            .subtract(outputLayerErrorTerm.apply(x -> learningRate * x));
    ...
    for (i = layerInProcessing - 1; i > 0; i--) {
      Matrix thisLayerErrorTerm =
          nnConfig
              ...
              .dot(nnConfig.getWeights().get(i + 1).transpose().cross(nextLayerErrorTerm));
      Matrix deltaWeightI =
          thisLayerErrorTerm.multiply(
              nnConfig.getLayerOutputs().get(i - 1).apply(Functions::sigmoid));
      newWeights = nnConfig.getWeights().get(i).subtract(deltaWeightI.apply(x -> learningRate * x));
      nnConfig.getWeights().set(i, newWeights);

      newBiases =
          nnConfig.getBiases().get(i).subtract(thisLayerErrorTerm.apply(x -> learningRate * x));

    ...
    private NeuralNetwork trainForOneInput(
        ...
        Matrix deltaWeightLast =
                outputLayerErrorTerm.multiply(
                        nnConfig.getLayerOutputs()
                                .get(layerInProcessing - 1)
                                .apply(Functions::sigmoid));
        ...
        for (i = layerInProcessing - 1; i > 0; i--) {
            ...
            newWeights =
                    nnConfig.getWeights()
                            .get(i)
                            .subtract(deltaWeightI.apply(x -> learningRate * x));
            nnConfig.getWeights().set(i, newWeights);

            newBiases =
                    nnConfig.getBiases()
                            .get(i)
                            .subtract(thisLayerErrorTerm.apply(x -> learningRate * x));
        ...
        newWeights =
                nnConfig.getWeights().get(i).subtract(deltaWeightI.apply(x -> learningRate * x));
        nnConfig.getWeights().set(i, newWeights);
